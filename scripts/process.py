...
def data_package():
    package = Package()
    package.infer(r'data/lifeExpectancy-final.csv')
    package.commit()
    package.save(r"datapackage.json")

workbook = openpyxl.load_workbook('archive/lifeExpectancy.xlsx')
worksheet = workbook.active
desired_columns = [1, 25]
new_values = ["Region","Value","Year"]
with open('archive/lifeExpectancy.csv', 'w', newline='', encoding='utf-8') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(new_values)
    for row in worksheet.iter_rows(min_row=9, max_row=28):
    ...
    with open(input_file, 'r', newline='') as infile, open(output_file, 'w', newline='') as outfile:
        ...
        for new_name, row in zip(new_names, reader):
            if row:  # Check if row is not empty
                row[0] = new_name
                writer.writerow(row)
input_file = 'archive/lifeExpectancy.csv'
output_file = 'data/lifeExpectancy-final.csv'
